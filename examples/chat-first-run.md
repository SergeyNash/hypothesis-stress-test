...
File-first fallback: [run.md](./run.md) и [example-001/](./example-001/).
...
Алиасы: `#discovery`, `#custdev`, `#новый-прогон`.

**Важно:** `runs/` создаётся после **двух** подтверждений: (1) draft карточки (2) предложенный `RUN_DIR`. До этого: `runs/ ещё НЕ создан`. Агент не должен писать в существующий run при новой гипотезе.
...
2. `.clinerules/` и `.cline/` в корне workspace — [implementations/quick-start.md](../implementations/quick-start.md)
3. Настроен Confluence MCP (рекомендуется) — [implementations/confluence-mcp.md](../implementations/confluence-mcp.md)
...
| RUN_DIR | Новый archive после dialog confirm | Создаёт пользователь |
| Подтверждение | Draft + предложенный RUN_DIR (два шага) | Неявное (пользователь сам написал файл) |
...
### Шаг 3 — Draft карточки → двойной confirm → bootstrap
...
Пользователь: **Подтвердить карточку**
...
Пользователь: **Подтвердить создание 002** → создаётся `runs/HYP-2026-06-23-002/`.

---

## Пример C — Вторая гипотеза в тот же день (`#новая`)

`runs/HYP-2026-06-23-001/` уже содержит outputs от предыдущей гипотезы.  
Новая гипотеза — агент должен предложить `002`, а не писать в `001`.
...
[новый discovery-контекст — не связан с 001]
```

### Ожидаемое поведение

1. Агент **не** использует `001/outputs/` как target для новой гипотезы
2. После confirm draft предлагает `runs/HYP-2026-06-23-002`
...
- Запись анализа в `runs/HYP-2026-06-23-001/`
- Пропуск диалога RUN_DIR и reuse открытых табов
- Создание файлов до confirm предложенного `RUN_DIR`
